   ...
   */
   ...
   if ( l -> head == NULL){
     ...
     if (cur -> next == NULL){
       ...
       if (newNode -> blk -> start <= cur -> blk -> start){
         newNode -> next = l -> head;
         l -> head = newNode;
       }
       else{
         while ( cur != NULL && newNode -> blk -> start ){
           prev = cur; 
           cur = cur -> next; 
         }
         prev -> next = newNode;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   ...
   int newSize = newNode ->blk->end - newNode->blk->start;
   int curSize;
   ...
   if(l -> head == NULL){
     ...
     if (cur-> next == NULL){
       ...
       while( cur != NULL && newSize >= curSize){
         prev = cur;
         cur = cur -> next;
         if (cur != NULL){
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
   ...
   node_t *cur = l -> head;
   while(cur){
     if(prev->blk->end +1 == cur->nlk->start ){
