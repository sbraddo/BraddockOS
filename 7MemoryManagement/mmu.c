...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    while(cur){
        if(cur->blk->end - cur->blk->start >= blocksize){
            blockNode = cur;
            break;
        }
        prev = cur;
    ...
    if (blockNode->blk->end != blkEnd){
        block_t *f = (block_t*)malloc(sizeof(block_t));
        f->pid = 0;
        f->start = blockNode->blk->end+1;
        f->end = blkEnd;
        if(policy == 1){
            list_add_to_back(freelist,f);
        }
        else if (policy == 2){
            list_add_ascending_by_blocksize(freelist,f);
        }
        else{
            list_add_descending_by_blocksize(freelist,f);
        }
    }
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    deBlk->pid = 0;
    if(policy == 1){
        list_add_to_back(freelist,deBlk);
    }
    else if (policy == 2){
        list_add_ascending_by_blocksize(freelist,deBlk);
    }
    else{
        list_add_descending_by_blocksize(freelist,deBlk);
    }
